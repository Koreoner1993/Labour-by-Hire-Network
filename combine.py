#!/usr/bin/env python3
"""
Combines the updated main app HTML + NFT.html into index.html.

The user supplied a newer, expanded version of the main app (stored as
new_base.html). This script:
  1. Takes new_base.html as the primary source
  2. Extracts NFT CSS/HTML/JS from NFT.html
  3. Inserts them into the new base
...
# 1. READ SOURCE FILES
# ──────────────────────────────────────────────────────────────────────────────
with open('new_base.html', 'r', encoding='utf-8') as f:
    base = f.read()
...
# ──────────────────────────────────────────────────────────────────────────────
# 4. BUILD COMBINED FILE
# ──────────────────────────────────────────────────────────────────────────────

combined = base

# A) Add Barlow fonts after DM Sans font link
...
# E) Update showView() to include 'nft'
combined = combined.replace(
    "['home','profile','listed','getlisted','employer-landing','dashboard','equipment','equipment-profile','spotlight','about'].forEach",
    "['home','profile','listed','getlisted','employer-landing','dashboard','equipment','equipment-profile','spotlight','about','nft'].forEach"
)

# F) Add NFT JS before </script>
nft_js_block = f"""
/* ══════════════════════════════════════════════════════════════════════════
...
print(f"Done! index.html written — {lines:,} lines · {size_kb:,.0f} KB")
